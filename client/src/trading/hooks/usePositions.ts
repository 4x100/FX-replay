    ...
    openPositions.forEach((pos) => {
      ...
      if (line) {
        try {
          ...
          // กราฟหายไปแล้ว (เช่น เปลี่ยน TF) ข้ามไปได้เลย
        }
      }
    });
    ...
    openPositions.forEach((pos) => {
      if (pos.type === "BUY") {
        if (pos.tp > 0 && currentPrice >= pos.tp)
          handleClosePosition(pos.id, "HIT TP");
        if (pos.sl > 0 && currentPrice <= pos.sl)
          handleClosePosition(pos.id, "HIT SL");
      } else {
        if (pos.tp > 0 && currentPrice <= pos.tp)
          handleClosePosition(pos.id, "HIT TP");
        if (pos.sl > 0 && currentPrice >= pos.sl)
  ...
  const handleTrade = (type: "BUY" | "SELL") => {
    ...
    if (seriesRef.current) {
      try {
        ...
        let tp, sl;
        if (tpPrice > 0)
          tp = seriesRef.current.createPriceLine({
            price: tpPrice,
            color: "#089981",
            lineWidth: 2,
        ...
        if (slPrice > 0)
          sl = seriesRef.current.createPriceLine({
            price: slPrice,
            color: "#f23645",
            lineWidth: 2,
            lineStyle: LineStyle.Solid,
